...
Run with the -testnet option to run with "play cashgoldcoins" on the test network, if you
...
CashGoldCoin Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- BitcoinMiner : Generates cashgoldcoins (if wallet is enabled).
...
However, in open source software such as CashGoldCoin Core, where everyone uses
...
pay attention to for reviewers of CashGoldCoin Core code.

General CashGoldCoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: CashGoldCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of CashGoldCoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against CashGoldCoin Core so that they can be integrated
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:cashgoldcoin-project/cashgoldcoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `cashgoldcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `cashgoldcoin-cli` and the GUI debug console use this table to determine how to
