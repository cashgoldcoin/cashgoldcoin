CashGoldCoin Core version *0.15.1* is now available from:

  <https://download.cashgoldcoin.org/cashgoldcoin-0.15.1/>
...
A number of changes to the way CashGoldCoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 cash per kB (0.00001 CGO). This is to prepare for dropping the minimum transaction fee to 0.1 cash per kB (0.0001 CGO) in the next release.
